// PoC innerHTML sanitization wrapper for Phase 5.1.
...
// returns the HTML unchanged and logs a warning. This is PoC-scoped behavior.
// Production builds must always load DOMPurify - the build pipeline copies it
// to dist/vendor/purify.min.js via vite-plugin-static-copy.
...
(function () {
  ...
  function warnOnce() {
    if (!warned) {
      console.warn('[SafeHTML] DOMPurify not loaded. PoC fail-open mode: HTML passes through unsanitized. Production must load DOMPurify from /vendor/purify.min.js (served by Vite build).');
      warned = true;
    }
  }
