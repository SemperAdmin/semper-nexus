...
(function () {
  ...
  // Failing closed blanks every card, so the only visible symptom is empty
  // rows while the counts still read correctly. The console warning below is
  // the sole signal for this state (the on-page banner was removed by
  // request): a deployment showing blank cards is serving the repository
  // root instead of the Vite build output, which is missing
