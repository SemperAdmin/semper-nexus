...
// missing. Production loads DOMPurify from vendor/purify.min.js, which
// vite-plugin-static-copy writes from node_modules at build time. Serving the
// repository root instead of the build output leaves that path empty.
...
(function () {
  ...
  function showBanner() {
    ...
    var render = function () {
      ...
      el.textContent = 'Content sanitizer failed to load, so all message content is suppressed. This deployment is missing vendor/purify.min.js. Serve the Vite build output rather than the repository root, then redeploy.';
      // Sticky, not relative: users reach the broken list only after scrolling
      // past the header, and a banner that scrolls away is never seen.
      el.style.cssText = 'background:#8b0000;color:#fff;padding:0.75rem 1rem;font-size:0.95rem;text-align:center;position:sticky;top:0;z-index:9999;';
