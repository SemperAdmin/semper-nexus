...
// missing. DOMPurify is committed at vendor/purify.min.js so both the Vite
// build and a directly-served repository root load it; refresh the committed
// copy with `npm run update-vendor`.
...
(function () {
  ...
  function showBanner() {
    ...
    var render = function () {
      ...
      el.textContent = 'Content sanitizer failed to load, so all message content is suppressed. This deployment is missing vendor/purify.min.js. Restore the vendor/ directory (npm run update-vendor) and redeploy.';
