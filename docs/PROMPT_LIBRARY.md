...
Adopted into this repository on 2026-07-04 per the Maintenance section below
("Version this file with the project"). The Shared Context Block is filled for
Semper Nexus from the code and configuration as they exist in this repo; each
line is labeled with its evidence tier per Global Rule 3.

## Shared Context Block — Semper Nexus (filled 2026-07-04)

```
PROJECT CONTEXT
Stack: Vanilla JavaScript (ES modules), no frontend framework. Vite ^5.0.0
  build with Terser minification (package.json, vite.config.js). PWA: service
  worker + manifest.json. Node 18 for build and data scripts
  (.github/workflows/deploy-github-pages.yml pins node-version 18). Companion
  backends: Express ^4.18.2 CORS proxy (proxy-server/, Node >=18) and a
  Cloudflare Worker proxy (cloudflare-worker/). No database — browser
  localStorage for caching plus committed static data files in lib/.
  [VERIFIED: package.json, proxy-server/package.json, README.md]
Deployment target: GitHub Pages for the static frontend (Actions workflow on
  push to main); Render for the Express proxy (proxy-server/render.yaml);
  Cloudflare Workers as the serverless proxy alternative.
  [VERIFIED: .github/workflows/, proxy-server/render.yaml]
Load profile: Not measured — no telemetry numbers exist in the repo. Known
  bound: Cloudflare Workers free tier caps the edge proxy at 100,000
  requests/day (cloudflare-worker/README.md). Frontend is static and served
  from GitHub Pages CDN. [ASSUMED: low single-digit RPS; update this line
  with real numbers when analytics exist.]
Data sensitivity: None stored server-side. Content is public military
  administrative messages (MARADMIN, NAVADMIN, ALNAV, SECNAV, JTR). No user
  accounts, no authentication, no PII persisted by the application.
  [VERIFIED: README.md; INFERRED from absence of any auth or user-data code]
Team: Single maintainer (SemperAdmin). No dedicated ops or on-call.
  [INFERRED: commit history and repo ownership]
Non-goals: No backend database, no user accounts or authentication, no
  server-side rendering. Frontend stays deployable as static files.
  [INFERRED: README.md "Frontend-only" architecture statement]
Acceptance criteria: Defined per task at invocation time. Repo-wide baseline:
  `npm run build` succeeds, `npm run lint` and `npm run format:check` pass.
  Note: no automated test suite exists — Mode 5's characterization-test
  precondition applies to any refactor. [VERIFIED: package.json scripts]
```
